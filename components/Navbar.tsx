...
interface NavbarProps {
	data?: ContactInformationResponse;
	footerRes?: WebsiteInformationResponse;
	socialRes?: SocialLinkResponse;
}


const Navbar = ({data,footerRes,socialRes}: NavbarProps)  => {
	// Type checking for contact info
	const isError = (data as ErrorResponse)?.error !== undefined;
	const contactInfo = !isError && data ? (data as ContactInformation) : null;;
        ...
        `}>
					<div className="flex items-center justify-between text-sm text-gray-500">
						<div className="flex items-center gap-4 mt-5">
							<a href={`tel:${contactInfo?.contactNumber}}`} className={'hover:text-indigo-800' +
								' transition-colors'}>
              <span className="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
	              ...
	              {contactInfo?.contactNumber}
							...
							<a className={'hover:text-indigo-800 transition-colors'} href={`mailto:${contactInfo?.email}`}>
                  ...
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                        d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
							...
							<h1 className={'text-lg font-semibold text-gray-600 mx-10'}>
								<span className={'text-indigo-800'}>Service Areas:</span> {contactInfo?.address}</h1>
						</div>
						<div className="hidden sm:flex items-center gap-5">
							<a href={socialData?.facebookLink} target={'_blank'} className="hover:text-blue-600 transition-colors">
							...
							<a href={socialData?.instagramLink} target={'_blank'} className="hover:text-pink-600 transition-colors">
								<svg className="w-10 h-10" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
									<path fillRule="evenodd"
